...
public class SistemaSolar extends Application {
	...
	public Point2D centro(Image img, double centroX, double centroY) {
		Point2D d = new Point2D(centroX - img.getWidth() / 2, centroY - img.getHeight() / 2);
		return d;
	}
	...
	public void start(Stage stage) throws Exception {
		...
		stage.setScene(scn);
		InputStream isEspaco = getClass().getResourceAsStream("/images/space.png");
		...
		new AnimationTimer() {
			...
			public void handle(long now) {
				...
				ctx.drawImage(imgSun, pointSun.getX(), pointSun.getY());
